...
function legacyEconomicAmount(gross, rate, treatment) {
    if (treatment !== "computable")
        return D(gross).toString();
    return D(gross).div(D(1).plus(D(rate))).toString();
}
function inferActivity(items) {
    const origins = new Set(items.filter((item) => item.vendible).map((item) => item.origen_item));
    if (origins.has("mixto") || (origins.has("fabricado") && origins.has("comprado")))
        return "mixto";
    return origins.has("fabricado") ? "fabricacion" : "reventa";
}
export function migrateLegacyCalculationInput(value) {
    const legacy = legacyCalculationInputSchema.parse(value);
    const condicionFiscal = "responsable_inscripto";
    const migrated = {
        ...
        items: legacy.items.map((item) => ({
            ...
            venta: item.venta === undefined ? undefined : {
                ...
                precio_neto_unitario: legacyEconomicAmount(item.venta.precio_bruto_unitario, item.venta.alicuota_iva, item.venta.tratamiento_iva ?? "computable"),
                ...(item.venta.descuento_bruto_total === undefined
                    ? {}
                    : {
                        descuento_neto_total: legacyEconomicAmount(item.venta.descuento_bruto_total, item.venta.alicuota_iva, item.venta.tratamiento_iva ?? "computable")
                    })
            }
        })),
    ...
    return calculationInputSchema.parse(migrated);
}
export function parseCalculationInput(value) {
    return calculationInputSchema.parse(value);
}
export function parseCalculationInputWithMigration(value) {
