...
function validateDecimal(
  ...
  options: { positive?: boolean; rate?: boolean } = {}
): void {
...
function validateItem(item: ItemInput, index: number, itemsById: Map<string, ItemInput>, issues: ValidationIssue[]): void {
...
function validateGraph(itemsById: Map<string, ItemInput>, issues: ValidationIssue[]): void {
  ...
  const visit = (itemId: string): number => {
    ...
    if (currentState === "visited") return depthMemo.get(itemId) ?? 0;
    ...
    for (const component of item?.receta?.componentes ?? []) {
      ...
      if (child?.receta) depth = Math.max(depth, 1 + visit(child.item_id));
    }
    stack.pop();
