...
export const SCHEMA_VERSION = "2026-07-27.beta1" as const;
...
function economicAmount(gross: Decimal, vatRate: Decimal, treatment: TratamientoIva): Decimal {
...
function presentItemResult(entry: InternalItemResult): ItemResult {
...
export function calculate(input: CalculationInput): CalculationOutcome {
  ...
  const totalCostsLoaded = sum(input.costos.map((cost) => D(cost.monto_total))).plus(
